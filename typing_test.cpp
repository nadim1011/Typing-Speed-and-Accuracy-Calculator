...
vector<vector<string>> showStr(5);
    ...
    showStr[1]={"done", "from", "nation", "document", "include", "tag"};
    showStr[2]={"jumps","a","cute","quick","brown","cat","fox","come","over","the","time","zoo","lazy","sleep","dog","done", "from", "nation", "document", "include" };
     ...
     if(choice==1)
       {
         ...
         system("CLS");
            ...
            for(auto it=showStr[2].begin();it!=showStr[2].end();it++)
            cout<<*it<<' ';
            cout<<"\n";
     ...
     clock_t en=clock();     //end clock
            ...
            int in_n=input.size();
               for(int j=0;j<in_n;j++)
                     {
            if(find(showStr[2].begin(),showStr[2].end(),input[j])!=showStr[2].end())
                  rightInput++;
                  }

    double wordCount=min(showStr[2].size(),input.size());   // calculating
